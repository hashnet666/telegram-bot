...
// Root endpoint
app.get('/', (req, res) => {
  res.json({ status: 'Hydra Telegram Bot is running' });
});
...
// Website messages
app.post('/website-message', async (req, res) => {
  try {
    const { message, name = 'Website User', userId } = req.body;
...
// Get messages (for website polling)
app.get('/messages/:conversationId', (req, res) => {
  // Simple response - in production, store messages in database
  res.json({
    messages: [],
...
async function handleTelegramMessage(msg) {
  ...
  // Handle commands
  if (text.startsWith('/')) {
    await handleCommand(chatId, userId, text, user, name);
    return;
  }

  // Handle user input based on state
  await handleUserState(chatId, userId, text, user, name);
}
...
// Start server
app.listen(PORT, () => {
  console.log(`Hydra Telegram Bot running on port ${PORT}`);
});
